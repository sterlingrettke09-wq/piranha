...
export function PiranhaIntro() {
  ...
  // School-of-piranhas canvas animation.
  useEffect(() => {
    if (phase === 'done') return
  ...
  // First scroll / touch / key dismisses the intro.
  useEffect(() => {
    if (phase !== 'intro') return
    const begin = () => setPhase('exiting')
    const onKey = (e: KeyboardEvent) => {
      if ([' ', 'Enter', 'Escape', 'ArrowDown', 'PageDown'].includes(e.key)) begin()
    }
    window.addEventListener('wheel', begin, { passive: true, once: true })
    window.addEventListener('touchmove', begin, { passive: true, once: true })
  ...
  return (
    <div
      className={`fixed inset-0 z-50 overflow-hidden bg-[#16110f] transition-[opacity,transform] duration-1000 ease-in-out ${
        phase === 'exiting' ? '-translate-y-8 opacity-0 pointer-events-none' : 'opacity-100'
      }`}
    >
      ...
      <button
        type="button"
        onClick={() => setPhase('exiting')}
        className="absolute right-5 top-5 text-xs uppercase tracking-[0.2em] text-piranha-bone/50 hover:text-piranha-bone"
      >
        Skip
      </button>
    </div>
