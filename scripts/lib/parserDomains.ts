// THE PARSER-DOMAIN SWEEP'S LIBRARY HALF — the targets and the classification.
// Nothing runs on import. The CLI is scripts/enumerate-parser-domains.ts.
//
// ⚠️ SPLIT OUT 2026-08-19, for the reason recorded in lib/zoneEnumeration.ts: a
// top-level `main()` in the same file meant importing TARGETS fired 23 live
// layer queries. Under vite-node the entry filename is absent from process.argv,
// so no guard can tell "imported" from "invoked" — the only reliable fix is a
// module with nothing to run.
//
...
import { readEnumeration } from './zoneEnumeration'
...
export async function distinctValues(url: string, field: string): Promise<string[] | null> {
...
/** How this sweep sorts a target's values. THE one definition — exported so a
 *  second reader (parcel-weight.ts) joins to the same notion of "gap" rather
 *  than re-deriving it. Two implementations of "unhandled" would drift, and the
 *  drift would be invisible: both would print plausible totals. */
export interface Classified {
  handled: string[]
  /** Unhandled AND declared out of scope — not counted toward the total. */
  excused: string[]
  /** Unhandled and unexplained. These are the gaps. */
  gaps: string[]
  /** True when `scopedTo` removes the WHOLE target from the total. */
  wholeTargetScoped: boolean
}

export function classify(t: Target, vals: string[]): Classified {
  const unhandled = vals.filter((v) => !t.handled(v))
  const handled = vals.filter((v) => t.handled(v))
  if (t.scopedTo) return { handled, excused: unhandled, gaps: [], wholeTargetScoped: true }
  const ps = t.partiallyScoped
  const excused = ps ? unhandled.filter((v) => ps.explains(v)) : []
  const ex = new Set(excused)
  return { handled, excused, gaps: unhandled.filter((v) => !ex.has(v)), wholeTargetScoped: false }
}
